...
from .basics import ParametersOperation


class Conv2D_Op(ParametersOperation):
    ...
    def _pad_2d_obs(self, inp: np.ndarray) -> np.ndarray:
    ...
    def _pad_2d_channel(self, inp: np.ndarray) -> np.ndarray:
        # inp dimensions are (num_channels, image_width, image_height)
        return np.stack([self._pad_2d_obs(channel) for channel in inp])
    

    def _get_image_patches(self, input_: np.ndarray) -> np.ndarray:
        img_batch_pad = np.stack([self._pad_2d_channel(obs) for obs in input_])

        patches = []
        img_height = img_batch_pad.shape[2]
        for h in range(img_height - self.param_size + 1):
            for w in range(img_height - self.param_size + 1):
                patch = img_batch_pad[:, :, h:h+self.param_size, w:w+self.param_size]
    ...
    def _output(self, inference: bool = False) -> np.ndarray:
        ...
        patches_reshape = (patches.transpose(1,0,2,3,4).reshape(batch_size, img_size, -1))

        param_reshape = (self.param.transpose(0,2,3,1).reshape(patch_size, -1))

        output_reshape = (np.matmul(patches_reshape, param_reshape).reshape(batch_size, img_height, img_height, -1).transpose(0,3,1,2))
    ...
    def _input_grad(self, output_grad: np.ndarray) -> np.ndarray:
        ...
        param_reshape = (self.param.reshape(self.param.shape[0], -1).transpose(1, 0))

        return (np.matmul(output_patches, param_reshape).reshape(batch_size, img_height, img_height, self.param.shape[0]).transpose(0, 3, 1, 2))
